...
public class Grabber implements Grab {
    ...
    public static class GrabJob implements Job {
        ...
        public void execute(JobExecutionContext context) {
            ...
            for (Post post : parse.list(HabrCareerParse.getPageLink())) {
                store.save(post);
            }
        }
    }
                ...
                while (!server.isClosed()) {
                    ...
                    try (OutputStream out = socket.getOutputStream()) {
                        ...
                        for (Post post : store.getAll()) {
                            out.write(String.valueOf(post.getId()).getBytes(Charset.forName("Windows-1251")));
                            out.write(System.lineSeparator().getBytes());
                            out.write(post.getTitle().getBytes(Charset.forName("Windows-1251")));
                            out.write(String.valueOf(post.getCreated()).getBytes(Charset.forName("Windows-1251")));
                            out.write(System.lineSeparator().getBytes());
